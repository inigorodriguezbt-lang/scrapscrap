...
**The paper wants volume: aim for 5 to 10 stories every cycle**, plus snippets
for everything worth knowing that does not make a story. A cycle with one or
two stories is a thin cycle; only file fewer when the material genuinely is
not there.
...
**Fresh.** This publishes every two hours. A story is about something that
happened in the last twelve hours, a snippet the last twenty-four, by the
source's own timestamp. Nothing days old. Every find has `age_hours`; work from the top.
...
edition** and that clears the sourcing rules, `want` clusters first. A `dull`
cluster runs too if it is real news; it just goes after the interesting ones.
Then go to the finds and the worklist for more.
...
a story. So does a fresh item from a single named outlet, filed as "X
reported" and saying nobody else has it yet. Open the artifact and report from
it: the model card, the README, the Space. Give it a `cluster_id` of `find-<slug>`, cite the artifact as the
source, attribute every capability claim to its maker, and say plainly that no
one else has tested it yet. A find that cannot carry four paragraphs becomes a snippet.
